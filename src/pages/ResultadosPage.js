...
const ResultadosPage = () => {
  ...
  // válido si la cantidad de slides cambia.
  ...
  }, [isComplete, currentSlide, handleNext]);


  // --- HELPERS DE RENDER ---

  // Escalado dinámico ajustado específicamente para 1, 3, 5 y hasta 11 filas
  // para evitar desbordamientos de pantalla en cualquier dispositivo.
  const getEscalaFila = (cantidadFilas) => {
    const filas = Math.max(cantidadFilas, 1);
    if (filas === 1) {
      return { fontSize: '3.6vh', numeroSize: '3.6vh', padding: '0.5vh 1.2vw' };
    }
    if (filas <= 3) {
      return { fontSize: '3.6vh', numeroSize: '3.6vh', padding: '0.5vh 1.2vw' };
    }
    ...
    // Hasta 11 filas (ej. Trebol de la Fortuna / 30 Millones)
    return { fontSize: '3.6vh', numeroSize: '3.6vh', padding: '0.5vh 1.2vw' };
  };
  ...
  // Separa el texto del título del número de premio seco para destacarlo
  // permitiendo flex-wrap si el nombre del premio es muy largo.
  const renderTituloPremio = (titulo, fontSize = null) => {
    const match = titulo.match(/(\d+)/);
    if (!match) {
      return (
        <span className="titulo-premio" style={fontSize ? { fontSize } : undefined}>
          {titulo}
        </span>
      );
    }
      ...
      >
        ...
        <span
          ...
          style={{
            fontWeight: 800,
            color: '#FFC94A',
            textShadow: '0 0 4px rgba(255, 201, 74, 0.5)',
          }}
        >
          {numero}
        </span>
      ...
      <aside className="sidebar-brand">
        ...
        >
          <img
            ...
            style={{ height: '15.5vh', width: 'auto' }}
          />
          <div
          ...
          />
          <img src={logoMoneda} className="logo-3d-small" alt="Logo" style={{ height: '15.5vh', width: 'auto' }} />
          <img src={textoLogo} className="logo-text-small" alt="Lotería" style={{ height: '5.5vh', width: 'auto' }} />
        </div>
                  ...
                  <table className="table-prizes">
                    <tbody>
                      {content.data.map((p, idx) => (
                        <tr key={idx}>
                          <td className="td-label" style={{ verticalAlign: 'middle', padding: escala.padding }}>
